...
const killCardHolderObserver = createMutationObserverForStylesIfDisplayBlock(
  setIsUserInDeathWindow
);
...
const inGameUiObserver =
  createMutationObserverForStylesIfDisplayBlock(setIsUserInGame);
...
function tryToActivateGame() {
  ...
  const node = localArray
    .arrayFrom(localDocument.getElementsByTagName("canvas"))
    .pop();
  node?.dispatchEvent(
    new localMouseEvent("mousedown", { bubbles: true, cancelable: true })
  );
  node?.dispatchEvent(
    new localMouseEvent("mouseup", { bubbles: true, cancelable: true })
  );
}
...
export function initFastRespawn() {
